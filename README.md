...
- `_.char16le(name, size, count)` — UTF-16BLE string. This has the same behavior as `_.char`. Node.js may truncate between surrogate pairs rather than Unicode code points, but if given an odd-length buffer will terminate with `\0` rather than half of a `.charCodeAt()` value.
